            ...
            <div class="timeline-v2" style="position: relative; max-width: 800px; margin: 0 auto; padding-left: 40px;">
                ...
                <div class="timeline-item" style="position: relative;">
                    ...
                    <div class="card timeline-card-v2 scratch-card-fixed" style="margin: 0; padding: 20px; border-radius: 20px; box-shadow: 0 10px 20px rgba(0,0,0,0.05); border-left: 5px solid #ff1493; background: #fff; text-align: center;">
                        ...
                        <p style="font-size: 0.7rem; color: #aaa; margin-top: 10px; font-style: italic;">Gosok layarnya pakai jari kamu ya!</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    case 'jigsaw':
            content.innerHTML = `
                <div class="card" style="text-align:center;">
                    <h3 class="romantic-text">Jigsaw Puzzle Foto 🧩</h3>
                    <p>Susun potongan foto kita ya sayang!</p>
                    <div id="puzzleBoard" class="puzzle-board"></div>
                    <button class="btn" onclick="initPuzzle()" style="margin-top:15px; background:#ff69b4; color:white; border:none; padding:10px 20px; border-radius:10px; cursor:pointer;">Acak Ulang</button>
                </div>
            `;
            initPuzzle(); // Panggil fungsi di luar switch
            break;
    ...
    content.innerHTML = `
        <div class=" dynamicCountdownBg" "card countdown-container" style="text-align: center;">
            ...
            <div class="timer-section past" style="width: 100%;">
                <p class="timer-label">Sudah Terlewati</p>
...
function initLoveCalendar(content) {
    // 1. Pindahkan Daftar tanggal spesial ke paling atas agar terbaca oleh sistem
    const specialDates = {
        "14-2": "Valentine Day 🌹",
        "15-7": "Selamat Ulang Tahun Sayang 🎂",
        "22-4": "Anniversary Kita 💖",
        "22-4-2025": "Hari dimana cerita kita dimulai bersama 🥰", 
        "15-7-2007": "Hari Dimana Orang Spesial Dilahirkan Yaitu Kamu ✨"
    };

    const monthNames = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
    
    // 2. Isi struktur HTML kalender
    content.innerHTML = `
        <div class="card">
            <div class="calendar-header" style="display: flex; flex-direction: column; gap: 10px; align-items: center; margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 5px; align-items: center;">
                    <input type="number" id="inputMonth" placeholder="Bulan" min="1" max="12" 
                           value="${currentMonth + 1}" 
                           style="width: 80px; padding: 8px; border-radius: 10px; border: 1px solid #ff69b4; outline: none;">
                    
                    <input type="number" id="inputYear" placeholder="Tahun" 
                           value="${currentYear}" 
                           style="width: 100px; padding: 8px; border-radius: 10px; border: 1px solid #ff69b4; outline: none;">
                    
                    <button onclick="manualJumpToDate()" 
                            style="padding: 8px 15px; border-radius: 10px; background: #ff69b4; color: white; border: none; cursor: pointer;">
                        Cari 🔍
                    </button>
                </div>
            </div>
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const specificKey = `${d}-${currentMonth + 1}-${currentYear}`; 

        let eventText = "";
        let isSpecial = false;

        if (specialDates[specificKey]) {
            eventText = specialDates[specificKey];
            isSpecial = true;
        } else if (specialDates[yearlyKey]) {
            if (yearlyKey === "22-4" && currentYear > 2025) {
                eventText = `Happy Anniversary yang ke-${currentYear - 2025} Sayang! 💖`;
            } else if (yearlyKey === "15-7") {
                eventText = `Happy Birthday Sayang ke-${currentYear - 2007} tahun! 🎂💖`;
            } else {
                eventText = specialDates[yearlyKey];
            }
            isSpecial = true;
        }
        ...
        grid.innerHTML += `
            <div class="calendar-day ${isSpecial ? 'special-day' : ''} ${isToday ? 'today' : ''}" 
                 onclick="${isSpecial ? `document.getElementById('eventNote').innerText='${eventText}'` : `document.getElementById('eventNote').innerText=''`}">
                ${d}
                ${isSpecial ? '<span style="display:block; font-size:10px;">❤️</span>' : ''}
            </div>`;
    }
}
...
function renderPuzzle() {
    ...
    puzzleOrder.forEach((pieceValue, index) => {
        ...
        if (pieceValue === 8) {
            ...
            // GANTI BAGIAN INI: Gunakan variabel currentPuzzleImage
...
// Daftar foto untuk Jigsaw Puzzle
const puzzleImages = [
    "Love.jpg",
    "Kenangan1.jpg",
    "Kenangan2.jpg",
...
let currentPuzzleImage = ""; // Menyimpan foto yang terpilih saat ini
...
function manualJumpToDate() {
    ...
    if (!m || m < 1 || m > 12) {
        ...
        alert("Bulan harus antara 1 sampai 12 ya sayang! 🌸"); // <--- Muncul jika bulan > 12 atau kosong
...
function startRealtimeCountdown() {
    ...
    if (noteEl) {
        ...
        if (s === 22) { // Anggap jadian tanggal 22
            secretMsg.innerText = "I Love You More! ❤️";
...
window.openEnvelope = function() {
    ...
    if (envelope) {
        // Animasi amplop membuka
        envelope.classList.add('is-open');

        // Tunggu animasi flap selesai (800ms) baru munculkan surat
        setTimeout(() => {
            if (overlay) {
